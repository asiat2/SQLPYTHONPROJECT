        ...
        if (lstAvailableDrivers is not None):
            ...
            if(len(lstAvailableDrivers) > 0):
                ...
                if('windows' in identifiedOS.lower()):
                    ...
                    if("SQL Server Native Client 11.0" in lstAvailableDrivers):
                        self.selectedDriver = "{SQL Server Native Client 11.0}"
                
                    if(self.dbIsTrustedMode == False):
                        #Trusted Mode can only be used with SQL Server Native Client
                        if("ODBC Driver 13 for SQL Server" in lstAvailableDrivers):
                            self.selectedDriver = "{ODBC Driver 13 for SQL Server}"

                        if("ODBC Driver 13.1 for SQL Server" in lstAvailableDrivers):
                            self.selectedDriver = "{ODBC Driver 13.1 for SQL Server}"
